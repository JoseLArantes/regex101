...
I want to extract the account number from an AWS ARN, The arn is in variable and I want it into another variable
In my use case, the arn is a eks cluster arn. It is not a generic regex, it works probably in the EKS cases and in no other is guaranted.
...
Notice that 'arn:aws:eks:' is 3 times repeting `([a-z:]{4})`, then 'us-west-1' is simple `[a-z0-9-]+` and the account number comes after the ':' with numbers only, `([0-9]+)`, the rest is simple `:.+`.

I've used sed to extract the value of the second group and with the rest of the command to put it in the aws_account variable
